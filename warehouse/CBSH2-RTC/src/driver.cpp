...
Instance* createInstance(const string& data_format, const string& map_file, const string& agent_file, 
    ...
    if ((data_format.compare("gripmap")) == 0) {
		...
		return new GeneralRoadmapInstance(map_file, agent_file, agent_idx, rows, cols);
    }
...
int main(int argc, char** argv)
{
	...
	desc.add_options()
		...
		("format,f", po::value<string>()->default_value("gripmap"), "whether use gripmap or roadmap format")
    ...
    if (cbs.solution_found && vm.count("outputPaths"))
        cbs.savePaths(vm["outputPaths"].as<string>(), vm["format"].as<string>());
