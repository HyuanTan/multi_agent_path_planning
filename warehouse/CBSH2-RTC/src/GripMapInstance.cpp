...
#include"GripMapInstance.h"

int RANDOM_WALK_STEPS = 100000;

GripMapInstance::GripMapInstance(const string& map_fname, const string& agent_fname, 
...
list<int> GripMapInstance::getNeighbors(int curr) const
{
	list<int> neighbors;
...
void GripMapInstance::printAgents() const
{
...
void GripMapInstance::printMap() const
{
...
void GripMapInstance::saveMap() const
{
	ofstream myfile;
...
void GripMapInstance::saveAgents() const
{
	ofstream myfile;
...
void GripMapInstance::generateConnectedRandomGrid(int rows, int cols, int obstacles)
{
	...
	/*moves_offset[GripMapInstance::valid_moves_t::WAIT_MOVE] = 0;
	moves_offset[GripMapInstance::valid_moves_t::NORTH] = -num_of_cols;
	moves_offset[GripMapInstance::valid_moves_t::EAST] = 1;
	moves_offset[GripMapInstance::valid_moves_t::SOUTH] = num_of_cols;
	moves_offset[GripMapInstance::valid_moves_t::WEST] = -1;*/
...
void GripMapInstance::generateRandomAgents(int warehouse_width)
{
...
bool GripMapInstance::addObstacle(int obstacle)
{
	if (my_map[obstacle])
		return false;
...
bool GripMapInstance::isConnected(int start, int goal) const
{
	std::queue<int> open;
...
int GripMapInstance::randomWalk(int curr, int steps) const
{
...
bool GripMapInstance::loadMap()
{
	...
	{
		cout << "Loading Nathan's benchmark map ................." << endl;
	...
	{
		std::cout << "Loading self-defined benchmark map ................." << std::endl;
		char_separator<char> sep(",");
		tokenizer<char_separator<char>> tok(line, sep);
		beg = tok.begin();
		num_of_rows = atoi((*beg).c_str()); // read number of rows
	...
	cout << "Loading Nathan's benchmark map get num_of_cols:" << num_of_cols << "  map_size:" << map_size << endl;
	my_map.resize(map_size, false);
...
bool GripMapInstance::loadAgents()
{
	using namespace std;
	using namespace boost;
