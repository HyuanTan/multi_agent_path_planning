...
void CBS::savePaths(const string &fileName, const string& dataFormat) const
{
    std::ofstream output;
    output.open(fileName, std::ios::out);
	if ((dataFormat.compare("gripmap")) == 0)
	{
		for (int i = 0; i < num_of_agents; i++)
		{
			output << "Agent " << i << ": ";
	...
	else if ((dataFormat.compare("roadmap")) == 0)
	{
		for (int i = 0; i < num_of_agents; i++)
		{
			output << search_engines[0]->instance.getAgentName(i) << ": ";
			for (const auto & t : *paths[i])
				output << search_engines[0]->instance.getIdName(t.location) << "(" << search_engines[0]->instance.getCoordinateX(t.location)
					<< "," << search_engines[0]->instance.getCoordinateY(t.location) << ")->";
			output << endl;
		}
	}
		...
		heuristic_helper(instance.getDefaultNumberOfAgents(), paths, search_engines, initial_constraints, mdd_helper)
{
	...
	cout << "Initialize CBS, get num_of_agents:" << num_of_agents << "  num_of_cols: " << instance.num_of_cols << "  map_size: "<< instance.map_size << endl;
	for (int i = 0; i < num_of_agents; i++)
	{
		// cout << "agent:" << i << "  start_locations:" << instance.start_locations[i] << "  goal_locations:" << instance.goal_locations[i] << endl;
		if (sipp)
