...
int main(int argc, const char *argv[])
{
    if(argc > 2)
    {
        ...
        map.get_map(argv[1], argv[4]); // argv[4], string, sepcial roadmap format
        Task task;
        if (strstr(argv[4], "robot123") != NULL) task.get_robot123_task(argv[2], map);
        else
        {
        ...
        if (strstr(argv[4], "robot123") != NULL) logger.write_to_log_path_robot123(solution, map);
        else logger.write_to_log_path(solution, map);
        logger.save_log();
    }
    else
    {
