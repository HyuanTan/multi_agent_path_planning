...
private:
    ...
    bool get_robot123_roadmap(const char* FileName);
public:
    Map(double size, int k){ agent_size = size; connectedness = k; }
    ~Map(){}
    int  get_size() const { return size; }
    bool get_map(const char* FileName, const char* map_format);
    ...
    int get_id(std::string id_name) const;
    double get_i (int id) const;
    double get_j (int id) const;
    std::string get_id_name(int id) const;
